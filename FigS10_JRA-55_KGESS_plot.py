...
vmax = [1, 1, 1, 1, 1, 1]
vmin = [-1, -1, -1, -1, -1, -1]
folder_path = 'C:/Users/fzjxw/Desktop/Datacomparison/Glb_0.25_0317/output/metrics'
...
for i, ax in enumerate(axs.flat):
    if i == 5:  # Streamflow scatter plot
        ...
        cbar = plt.colorbar(cs1, ticks=ticks[i][::3], boundaries=boundaries[i], 
                           shrink=0.7, pad=0.01, orientation='vertical', extend='min')
        cbar.ax.tick_params(labelsize=colorbar_ticklabel_size, width=2, length=7)
    else:  # Other variables (imshow)
        data1 = xr.open_dataset(
            f'{folder_path}/{var[i]}_ref_{ref_data[i]}_sim_JRA55_0.25_KGESS.nc')['KGESS']
...
plt.savefig(
    'C:/Users/fzjxw/Desktop/study/BaiduSyncdisk/JRA3Q&JRA55/plot_code/Fig5_JRA-55_KGESS.jpg', 
