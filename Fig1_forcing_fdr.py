...
for i, ax in enumerate(axs.flat):
    ...
    if i == 3:  # spfh 单位转换 kg/kg -> g/kg
        data_std = xr.open_dataset(f'/media/zhwei/data01/weizx/forcing_DAY/stdmean/{vars[i]}_stdmean.nc')['std_mean'] * 1000
    else:
        data_std = xr.open_dataset(f'/media/zhwei/data01/weizx/forcing_DAY/stdmean/{vars[i]}_stdmean.nc')['std_mean']
    ...
    # 读取显著性掩码
    data_sig = xr.open_dataset(f'/media/zhwei/data01/weizx/forcing_DAY/pvalue/ind/fdr/fdr_{vars[i]}.nc')
    significant_mask = data_sig['significant_mask']
    lon1 = data_sig.lon
    lat1 = data_sig.lat
    
    # 从 CSV 获取 proportion_passed_fdr
    proportion = proportion_dict.get(vars[i], 0)
    
    # 绘制图像
    im = ax.imshow(dif_std, cmap=WhiteBlueGreenYellowRed, vmin=vmin[i], vmax=vmax[i], extent=[-180, 180, -60, 90])
    ...
    ax.text(0, 0.005, f'{round(proportion, 2)}%', fontsize=55, transform=ax.transAxes)
    ...
    # 添加色条
    cbar = plt.colorbar(im, ticks=ticks[i][::2], boundaries=boundaries[i], 
...
# 保存图像
plt.savefig('/media/zhwei/data01/weizx/forcing_DAY/Figure1_fdr_forcing.jpg', format='jpg', dpi=300, bbox_inches='tight')
print('出图完毕')