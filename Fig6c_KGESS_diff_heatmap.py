...
for k in range(0, len(file_lists), 2):
    ...
    # 计算差异 (JRA3Q - JRA55)
    df_diff = df1 - df2
    if df_diff.isna().any().any():
        print(f"Warning: NaN values in {var_name} after subtraction:\n", df_diff.isna().sum())
    kgess_diff_df.loc[var_name] = df_diff.values[0]  # 存储到 DataFrame 中
...
for i, (var_name, ax) in enumerate(zip(variables, axes)):
    ...
    # 画热图但不加默认 colorbar
    heatmap = sns.heatmap(row_data, ax=ax, cmap=cmaps[i], annot=True, fmt=".3f", linewidths=1, annot_kws={'size': font_size-2.5},
                          cbar=False, vmin=vmin, vmax=vmax, yticklabels=False)
    ...
    # 在右侧添加横向 colorbar
    cbar_ax = ax.inset_axes([1.02, 0.4, 0.07, 0.15])  # [右侧偏移, 纵向位置, 宽度, 高度]
    cbar = fig.colorbar(heatmap.collections[0], cax=cbar_ax, orientation='horizontal',
                        ticks=[vmin, vmax], extend='both')
    cbar.ax.tick_params(labelsize=font_size-2, direction='in', length=5)

# 设置标题和布局
fig.text(0.15, 0.98, '(c)', fontsize=35, va='top', ha='left')
plt.suptitle('KGESS difference between JRA-3Q and JRA-55', fontsize=35)
fig.subplots_adjust(hspace=0, left=0.15, right=0.85)

# 保存图像
plt.savefig('C:/Users/fzjxw/Desktop/Datacomparison/Glb_0.25_0317/output/metrics/IGBP_groupby/KGESS_all_diff.jpg',
