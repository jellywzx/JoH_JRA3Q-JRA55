...
# 定义文件路径和标签
file_paths = [
   folder_path+'/JRA3Q/Streamflow_ref_2903480_2001_2010.nc',
   folder_path+'/JRA3Q/Streamflow_sim_2903480_2001_2010.nc',
   folder_path+'/JRA55/Streamflow_sim_2903480_2001_2010.nc',
]
...
xtick_positions = [1 + i * 12 for i in range(10)] + [121]  # 每年的第一个月（2001-2011）加 2011
...
ax.set_yticks(np.linspace(0,7500,11)[::2])
...
# 添加文本
ax.text(0, 1.05, '(b) Station ID: 2903480 (Lat: 59.38°N, Lon: 121.38°E) ', fontsize=50, ha='left', transform=ax.transAxes)
ax.text(0.23, 0.7, 'RMSE: 698.12\nR: 0.87\nKGESS: 0.90', fontsize=40, ha="left", color = 'steelblue',  transform=ax.transAxes)
ax.text(0.55, 0.7, 'RMSE: 715.88\nR: 0.86\nKGESS: 0.90', fontsize=40, ha="left", color = 'palevioletred',  transform=ax.transAxes)
ax.text(0.85, 0.8, 'Asia',fontsize=60, ha="left",color = 'black', transform=ax.transAxes)
# 调整布局并保存
# plt.subplots_adjust(
    # left=0.05, 
                    # right=0.95, top=0.95, bottom=0.05)
plt.savefig(folder_path+'/2903480.jpg', format='jpg', dpi=300)
