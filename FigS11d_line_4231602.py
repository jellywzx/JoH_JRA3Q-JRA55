...
# 定义文件路径和标签
file_paths = [
   folder_path+'/JRA3Q/Streamflow_ref_4231602_2001_2010.nc',
   folder_path+'/JRA3Q/Streamflow_sim_4231602_2001_2010.nc',
   folder_path+'/JRA55/Streamflow_sim_4231602_2001_2010.nc',
]
...
ax.set_yticks(np.linspace(0,3500,11)[::2])
...
# 添加文本
ax.text(0, 1.05, '(d) Station ID: 4231602 (Lat: 47.12°N, Lon: -67.88°E) ', fontsize=50, ha='left', transform=ax.transAxes)
ax.text(0.23, 0.7, 'RMSE: 369.09\nR: 0.68\nKGESS: 0.72', fontsize=40, ha="left", color = 'steelblue',  transform=ax.transAxes)
ax.text(0.5, 0.7, 'RMSE: 423.68\nR: 0.75\nKGESS: 0.60', fontsize=40, ha="left", color = 'palevioletred',  transform=ax.transAxes)
ax.text(0.75, 0.85, 'North America',fontsize=60, ha="left",color = 'black', transform=ax.transAxes)
# 调整布局并保存
plt.subplots_adjust( right=0.95, top=0.95, bottom=0.05)
plt.savefig(folder_path+'/4231602.jpg', format='jpg', dpi=300)

# # 裁剪图片
# import subprocess
# subprocess.run([
#     '/stu01/huangzl/software/miniconda3/bin/magick',
#     '/stu01/baif/biyelunwen/code/runoff/line_Psand.jpg',
#     '-trim',
#     '/stu01/baif/biyelunwen/code/runoff/line_Psand.jpg'
# ])